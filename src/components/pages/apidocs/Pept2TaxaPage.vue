...
const doRequest = async () => {
    tryItResponse.value = await unipeptCommunicator.pept2taxa(input.value.split('\n'), equate_il.value, extra.value, names.value);
}

onBeforeMount(async () => {
    response1.value = await unipeptCommunicator.pept2taxa(["AIPQLEVARPADAYETAEAYR"]);
    response2.value = await unipeptCommunicator.pept2taxa(["AIPQLEVARPADAYETAEAYR", "APVLSDSSCK"]);
    response3.value = await unipeptCommunicator.pept2taxa(["APVLSDSSCK"], true, undefined, undefined);
    response4.value = await unipeptCommunicator.pept2taxa(["AIPQLEVARPADAYETAEAYR"], undefined, true, undefined);
    response5.value = await unipeptCommunicator.pept2taxa(["AIPQLEVARPADAYETAEAYR"], undefined, true, true);
})
</script>
